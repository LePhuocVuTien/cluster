  ...
  if ([self shouldRenderAsCluster:cluster atZoom:zoom]) {
    ...
    for (id<GMUClusterItem> item in cluster.items) {
      ...
      if ([item class] == [MFMarker class]) {
        marker = (MFMarker<GMUClusterItem> *)item;
        marker.map = _mapView;
      } else {
        CLLocationCoordinate2D fromPosition = kCLLocationCoordinate2DInvalid;
...
- (MFMarker *)markerWithPosition:(CLLocationCoordinate2D)position
  ...
  if (clusterIcon != nil) {
    //marker.iconView = clusterIcon;
    marker.iconView = [[UIImageView alloc] initWithImage:clusterIcon];
...
- (MFCoordinateBounds *)visibleRegionBounds:(CLLocationCoordinate2D)position withZoom:(float)zoom {
  MFCameraPosition *camera = _mapView.camera;
  [camera setTarget:position];
  [camera setZoom:zoom];
  
  CGPoint p = [[_mapView projection] pointForCoordinate:position camera:camera];
  double mViewWidth = UIScreen.mainScreen.bounds.size.width;
  double mViewHeight = UIScreen.mainScreen.bounds.size.height;
  double halfWidthSpan = mViewWidth / pow(2, zoom) / 256 / 2;
  double halfHeightSpan = mViewHeight / pow(2, zoom) / 256 / 2;
  CGPoint pX = CGPointMake(p.x - halfWidthSpan, p.y - halfHeightSpan);
  CGPoint pY = CGPointMake(p.x + halfWidthSpan, p.y + halfHeightSpan);
  
  CLLocationCoordinate2D x = [[_mapView projection] coordinateForPoint: pX];
  CLLocationCoordinate2D y = [[_mapView projection] coordinateForPoint: pY];
  MFCoordinateBounds *bounds = [[MFCoordinateBounds alloc] initWithCoordinate:x coordinate1:y];
